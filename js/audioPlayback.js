function AudioPlayback() {
  ...
  this.onAudioUpdate = function onAudioUpdate(evt) {
    ...
    if (audioData.length == 1) {
      // Mono
      audioPlayback.copyChannelDataToBuffer(
        leftBuffer,
        audioData[0],
        audioPlayback.currentPlayPosition,
        bufferSize,
        audioPlayback.playStart,
        audioPlayback.playEnd
      );
      audioPlayback.currentPlayPosition = audioPlayback.copyChannelDataToBuffer(
        rightBuffer,
        audioData[0],
        audioPlayback.currentPlayPosition,
        bufferSize,
   ...
   */
  this.copyChannelDataToBuffer = function copyChannelDataToBuffer(
    bufferReference,
    dataReference,
    position,
    len,
    ...
    var firstSplitEnd =
      position + len > dataReference.length
        ? dataReference.length
        : position + len > endPosition
        ? endPosition
        : position + len;
    var firstSplitLen = firstSplitEnd - firstSplitStart;
    var secondSplitStart =
      firstSplitLen < bufferReference.length ? 0 : undefined;
    var secondSplitEnd =
      secondSplitStart !== undefined
        ? bufferReference.length - firstSplitLen + secondSplitStart
        : undefined;
    var secondSplitOffset =
      bufferReference.length - (firstSplitEnd - firstSplitStart);
   ...
   */
  this.copyIntoBuffer = function copyIntoBuffer(
    bufferReference,
    bufferOffset,
    dataReference,
    dataOffset,
  ...
  this.play = function play(audioDataRef, sampleRate, start, end) {
    ...
    if (
      this.isPlaying ||
      audioDataRef === undefined ||
      audioDataRef.length < 1 ||
    ...
    this.playStart =
      start === undefined || start < 0 || start >= audioDataRef[0].length
        ? 0
        : start;
    this.playEnd =
      end === undefined ||
      end - this.audioBufferSize < start ||
      end >= audioDataRef[0].length
  ...
  //TODO: ADDED: PAUSE
  this.pauseResume = function pauseResume(audioDataRef) {
    if (
      this.audioDataRef === undefined ||
      this.audioDataRef.length < 1 ||
    ...
    if (this.isPlaying === false) {
      ...
      this.javaScriptNode.disconnect(this.audioContext.destination);
      this.isPlaying = false;
    }
  };
  ...
  this.getCurrentTime = function getCurrentTime() {
    return (
      this.javaScriptNode.eventHost.currentPlayPosition /
      this.javaScriptNode.eventHost.playEnd
    );
  };
