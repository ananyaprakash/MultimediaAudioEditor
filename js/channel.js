...
//only keep the 0.2s version, loop it to play the animation
function Channel(elementContext) {
  ...
  this.canvasWidth = elementContext.parentNode.parentNode.clientWidth - 4;

  // Colors
  this.backgroundColor = "#17a2b8"; // Background color
  this.plotColor = "#FFF"; // Color used to plot the waveform
  this.overlayLineColor = "#000";
  this.overlayTextColor = "#000";
  ...
  //TODO: ADD VAR
  this.currentTime = undefined; //CURRENT TIME OF THE PLAYING AUDIO
  ...
  this.createDisplay = function createDisplay() {
    ...
    this.canvasReference.style["border"] = "2px solid #000";
    ...
    if (length > this.canvasReference.width) {
      ...
      for (var i = 0; i < this.canvasReference.width; ++i) {
        ...
        if (
          ...
          // The start or end of the set of data points are outside the valid range
  ...
  this.paintOverlayMarkings = function paintOverlayMarkings(canvasContext) {
    ...
    t = this.toDP(
      this.getSampleToSeconds(this.viewPos + this.viewResolution),
      4
    );
    ...
    t = this.toDP(
      this.getSampleToSeconds(this.viewPos + this.viewResolution / 2),
      4
    );
  ...
  this.zoomToSeconds = function zoomToSeconds(numberOfSeconds, zoomStartFrom) {
    var numberOfSamples =
      numberOfSeconds * this.audioSequenceReference.sampleRate;
    this.zoom(
      numberOfSamples,
      zoomStartFrom * this.audioSequenceReference.sampleRate
    );
  };
  ...
  this.zoomToCycles = function zoomToCycles(numberOfCycles, zoomStartFrom) {
    var samplePerCycle =
      ...
      parseInt($("#waveform-frequency").val());
  ...
  this.generateWaveform = function generateWaveform() {
    ...
    var newWaveformAudioSequence = WaveformGenerator.generateWaveform(
      selectedWaveType,
      freq,
      amp,
      duration
    ...
    var newAudioSequenceReference = new CreateNewAudioSequence(
      newWaveformAudioSequence
    );
  ...
  this.generateMusic = function generateMusic(data) {
    // Gather the basic parameters
    var selectedWaveType = "sine-time";
    var stereoPosition = 0.5;
    ...
    for (var i = 0; i < data.length; ++i) {
      ...
      newWaveformAudioSequence = WaveformGenerator.generateWaveform(
        selectedWaveType,
        freq,
        noteAmp,
        data[i].duration + durationAdjustment
      );
      ...
      for (
        var j = 0;
        j < newWaveformAudioSequence.length &&
        startIndex + j < newMusicAudioSequence.length;
        ++j
      ) {
  ...
  //TODO: ADD FUNC, LOOP THE TIME STROKE
  this.drawTimeStroke = function drawTimeStroke(percentage, text) {
    console.log("percentage", percentage);
    var canvasContext = this.canvasReference.getContext("2d");

    canvasContext.moveTo(percentage * this.canvasWidth, 50);
    canvasContext.lineTo(percentage * this.canvasWidth, this.canvasHeight - 50);
    canvasContext.fillText(
      text,
      percentage * this.canvasWidth,
  ...
  if (
    typeof this.elementContext.attributes.title !== undefined &&
